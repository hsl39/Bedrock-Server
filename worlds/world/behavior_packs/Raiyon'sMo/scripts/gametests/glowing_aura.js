...
system.runInterval(() => {
    ...
    entities.forEach(entity => {
        ...
        world.getDimension(entity.dimension.id).getBlock({x:x,y:entity.location.y +1,z:z}).typeId.startsWith('minecraft:light_block')) ? entity.location.y + 1 :
    
        (world.getDimension(entity.dimension.id).getBlock({x:x,y:entity.location.y +2,z:z}).typeId === 'minecraft:air' ||
        world.getDimension(entity.dimension.id).getBlock({x:x,y:entity.location.y +2,z:z}).typeId.startsWith('minecraft:light_block')) ? entity.location.y +2 : null)
